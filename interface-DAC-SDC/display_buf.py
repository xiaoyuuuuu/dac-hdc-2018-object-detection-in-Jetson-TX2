...
MyIP = ""
# 如果sender在windows上，ll类型的长度与linux上的接收端长度不一致。
SENDER_ON_WIN = False
write_index = 0
...
class process_display_Object:
    def __init__(self, addr_port_client_Img=("", 1000)):
        ...
        # 接收图片的线程
        self.receive_thread = threading.Thread(target=self.ProcessImg)
        # 显示画框图片的线程
        self.show_thread = threading.Thread(target=self.show_image)
        print 'displayImg_Connect_Object init'
        self.resolution = [640, 360]
        self.client_port_Img = addr_port_client_Img
        # 标准库里的queue线程安全
        self.q = Queue(BATCH_SIZE * 3)
        # 用来展示图片的线程
        self.show_queue = Queue(BATCH_SIZE * 4)
        # 下面用来追踪的。目前去掉了这部分功能
        self.q_list = []
        self.q_center = []
    ...
    def get_q_xy_mean(self, current_point):
        ...
        for point in self.q_list:
            ...
            mean_x += temp_center_x
            mean_y += temp_center_y
    ...
    def show_image(self):
        ...
        while 1:
            try:
                image = self.show_queue.get(timeout=5)
                print("show")
                ...
                # cv2.imwrite("/home/nvidia/pics/%04d.jpg" %(write_index), image.image)
                # write_index+=1
            finally:
                if cv2.waitKey(5) == 27:
            ...
            try:
                ...
                for i in range(BATCH_SIZE):
                    ...
                    if confidence:
                        ...
                        image_origin[i].detecRec = [0, 0, 0, 0]
                    self.show_queue.put(image_origin[i])
    ...
    def thread_run(self):
        ...
        self.show_process = multiprocessing.Process(target=self.show_image)
