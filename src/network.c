...
extern image *resized_image;

//put the num's layer to CPU
int layer_number_in_cpu_post = 0;
...
void forward_network_cpu(network *netp)
{
    network net = *netp;
    int i;
    for(i = 0; i < net.n; ++i){
...
void *network_predict_pthread(pthread_args *arg_t){

	sem_wait(&sem_pic);
    arg_t->input_data = resized_image[0].data;
    network orig = *arg_t->net;
    arg_t->net->input = arg_t->input_data;
    arg_t->net->truth = 0;
    arg_t->net->train = 0;
    arg_t->net->delta = 0;
    forward_network(arg_t->net);
    *arg_t->net = orig;
    return NULL;
}

void *network_predict_pthread1(pthread_args *arg_t){

	sem_wait(&sem_pic1);
    arg_t->input_data = resized_image[1].data;
    network orig = *arg_t->net;
    arg_t->net->input = arg_t->input_data;
    arg_t->net->truth = 0;
    arg_t->net->train = 0;
...
void *network_predict_pthread2(pthread_args *arg_t){

	sem_wait(&sem_pic2);
    arg_t->input_data = resized_image[2].data;
    network orig = *arg_t->net;
    arg_t->net->input = arg_t->input_data;
    arg_t->net->truth = 0;
    arg_t->net->train = 0;
...
{
    // put some layers to GPU 
    network net = *netp;
	layer l;
    int i=0;
    cuda_set_device(net.gpu_index);
    cuda_push_array(net.input_gpu, net.input, net.layers[i].inputs*net.layers[i].batch);
    if(net.truth){
        cuda_push_array(net.truth_gpu, net.truth, net.truths*net.batch);
    }

    for(; i<net.n-layer_number_in_cpu_post; ++i){
        net.index = i;
        l = net.layers[i];
        l.forward_gpu(l, net);
	...
	if(layer_number_in_cpu_post != 0){
		// the reverse second layer
		l = net.layers[i];
